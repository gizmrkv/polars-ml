...
from const import END_INSERTION_MARKER, OPTIMIZE_TRANSFORMERS, START_INSERTION_MARKER
...
"""
    for method_name, transformer_cls in OPTIMIZE_TRANSFORMERS:
...
def insert_optimize_methods():
    ...
    insert_between_markers(
        target_file,
        "".join(codes),
        START_INSERTION_MARKER.format(prefix=" " * 4, suffix=" IN OptimizeNameSpace"),
        END_INSERTION_MARKER.format(prefix=" " * 4, suffix=" IN OptimizeNameSpace"),
    )


if __name__ == "__main__":
    insert_optimize_methods()
